...
function createSpin() {
  $.each($(spinnerSelector), (index, spinner) => {
    $(spinner).TouchSpin({
      ...
      // horizontalupclass: 'material-icons touchspin-up',
      // horizontaldownclass: 'material-icons touchspin-down',
...
$(document).ready(() => {
  ...
  prestashop.on('updatedCart', () => {
    ...
    try {
      $(".quickview")[0].classList.remove("in")
  ...
  const handleCartAction = (event) => {
    ...
    if(cartAction.type == 'deleteFromCart'){
      $target[0].innerHTML = `
      <span class="material-icons-outlined" aria-hidden="true" style="animation: spin 2s linear infinite;font-size: 20px;">cached</span>
      `
    };
    // return false;
    const requestData = {
  ...
  function updateProductQuantityInCart(event) {
    ...
    if (targetValue === '0') {
      $target.closest('.product-line-grid-body').find('[data-link-action="delete-from-cart"]').click();
    } else {
      $('.loader-con').css("display",'flex');
