...
<div class="">
    <h1 class="heading text-xs-center"><span>{l s='My wishlists' mod='blockwishlist'}</span></h1>
    <div id="mywishlist" class="pro-space">
        {* {capture name=path}
        ...
        {if isset($errors) && $errors}
            <div class="alert alert-danger">
                <p>{if $errors|@count > 1}{l s='There are %d errors' mod='blockwishlist' sprintf=$errors|@count}{else}{l s='There is %d error' mod='blockwishlist' sprintf=$errors|@count}{/if}</p>
                <ol>
                ...
                {if isset($smarty.server.HTTP_REFERER) && !strstr($request_uri, 'authentication') && preg_replace('#^https?://[^/]+/#', '/', $smarty.server.HTTP_REFERER) != $request_uri}
                    <p class="lnk"><a class="alert-link" href="#" title="{l s='Back' mod='blockwishlist'}">&laquo; {l s='Back' mod='blockwishlist'}</a></p>
                {/if}
            </div>
        {/if}
        ...
        {if $id_customer|intval neq 0}
            <form method="post" class="std box" id="form_wishlist">
                <fieldset>
                    <h4 class="page-heading">{l s='New wishlist' mod='blockwishlist'}</h4>
                    <div class="form-group">
                        <input type="hidden" name="token" value="{$token|escape:'html':'UTF-8'}" />
                        <label class="align_right " for="name">
                            {l s='Name' mod='blockwishlist'}
                        </label>
                        <input type="text" id="name" name="name" class="inputTxt form-control" value="{if isset($smarty.post.name) and $errors|@count > 0}{$smarty.post.name|escape:'html':'UTF-8'}{/if}" placeholder="{l s='Enter name' d='Shop.Theme'}" />
                    </div>
                    <p class="submit">
            ...
            {/if}
        {/if}
        <div class="footer_links wishlist_footer clearfix text-xs-right">
            
                <a class="btn btn-primary  button button-small pull-xs-right" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">
                    <span>
                        {l s='Back to Your Account' mod='blockwishlist'}
                    </span>
                </a>
                <a class="btn btn-default button button-small hidden-sm-down hidden-md-up" href="#">
                    <span>
